...
`SessionEvent` gains ADR-0007 tags with JSON strings and `EventId`s. `makeHost` provides `SessionLog` into `setup` and `ctx.contributions`. Tools stay `contribute(ToolKind)`. Each tool `execute`s a JSON argument string. A model plugin `provide`s effect-uai `LanguageModel`. Inference lists that token in `needs`. `send` appends `message/appended` and drains `foldThread` / `workOf` until `Idle`.
