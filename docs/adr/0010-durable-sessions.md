...
- A fork inherits its source's `thread/configured` fact, written by `Inference.fork` rather than by the RPC that asks for the fork, so every producer of a branch gets a fork that runs as its source did.
