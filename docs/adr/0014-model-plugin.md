...
- **Model plugin providing `LanguageModel`** (chosen): same `definePlugin` / `provide` path as every other service. Inference stays blocked until a model plugin is active. A later engine that does not use effect-uai simply does not `need` this token.
...
- Two plugins that `provide(LanguageModel)` still fail `DuplicateProvider`. Swap the model with `host.replace` or deactivate-then-activate.
