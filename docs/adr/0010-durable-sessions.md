...
- The journal's append order is what `parentId` and `seq` are stamped from. The SQL journal orders its rows by millisecond, so facts sharing one are the common case, and their relative order is the row order the driver returns. `session-log-sql.test.ts` pins that with one frozen timestamp for every fact, which turns a dependency on the driver's tie order into a test.
- `ThreadBranched.summary` still has no producer. It stays because the bridge that can fill it is the one that keeps branch summaries of its own, and dropping it would make a branch that leaves facts behind unable to say what it left.
- `HarnessForkRequest.checkpoint` is not carried: the RPC forks at the source's leaf, and a bridge's checkpoint id is its own vocabulary rather than oru's entry ids.
- The fork's lane is recorded before the bridge copies its own session, so a bridge that cannot copy leaves the lane forked and the RPC reports the failure. A caller that retries gets a second lane.
- A compaction cut can name a fact that renders as nothing, such as the branch marker of a fork with no request of its own. The model then sees the summary and that marker, which is the honest result of compacting a conversation with nothing in it.
- A compaction that a bridge cannot provide fails as a defect rather than as a described error, because `CompactThread` has no error channel. Surfacing it is a change to the RPC contract and belongs with the rest of the error vocabulary.
...
- A fork inherits its source's `thread/configured` fact, written by `Runtime.fork` rather than by the RPC that asks for the fork, so every producer of a branch gets a fork that runs as its source did.
