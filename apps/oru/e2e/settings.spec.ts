...
test('the Projects page renames a project the host recorded', async ({ page }) => {
  ...
  await page.locator('#settings-project-create-name').fill('rename-me')
  ...
  await page.locator('[data-projects-row]', { hasText: 'rename-me' }).click()
  await expect(page.locator('[data-project-detail-name]')).toContainText('rename-me')
  await page.locator('[data-projects-edit]').click()
  await page.locator('#settings-project-name').fill('renamed')
  await page.locator('[data-projects-save]').click()

  await expect(page.locator('[data-project-detail-name]')).toContainText('renamed')
  await page.reload()
  await expect(page.locator('[data-project-detail-name]')).toContainText('renamed')
})
