...
- **A WebSocket transport** (`RpcServer.layerHttp` defaults to it, and `Socket.layerWebSocketConstructorGlobal` needs no browser dependency): it keeps a ping/pong keepalive, retries with backoff, and gates chunk delivery on the client's ack, so a host that dies is noticed rather than hung on. It lost on evidence: a two-route probe with a streamed response never emitted, with no error and no output, and the robustness it adds, reconnect plus a resync marker so a replayed snapshot does not duplicate a transcript, is a change of its own. HTTP is proven end to end and through the dev proxy.
...
- The browser bundle drops from 554.47 kB to 519.89 kB (175.75 to 165.34 kB gzip), because the app no longer carries the kernel, the plugin set, or effect-uai.
